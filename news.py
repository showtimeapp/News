...
#                         st.session_state.current_page = next_page
#                     #     st.success(f"Added {new_results_count} new articles from page {next_page}")                       
#                     else:
...
# Custom CSS with improved aesthetics
st.markdown("""
    <style>
    /* Base table styles */
    .news-table {
    ...
    .app-header {
        text-align: center;
        color: #fff;
        background: linear-gradient(135deg, #262730, #1E212A);
        padding: 15px;
        border-radius: 10px;
        margin-bottom: 20px;
        box-shadow: 0 4px 6px rgba(0,0,0,0.1);
...
# Header - Inputs
st.markdown('<div class="section-header"><h3>Search Filters</h3></div>', unsafe_allow_html=True)
# st.subheader("Search Filters")
col1, col2, col3 = st.columns([3, 2, 2])
with col1:
    query = st.text_input("Enter search keyword", "Dharavi Redevelopment")
...
search_col1, search_col2 = st.columns([0.2, 1])
with search_col1:
...
# Source filter (shows up after first search)
if st.session_state.sources_list:
    # st.markdown('<div class="section-header"><h3>Source Filter</h3></div>', unsafe_allow_html=True)
    st.session_state.selected_sources = st.multiselect(
        "Select news sources to display", 
        options=st.session_state.sources_list,
        default=[]
...
if not st.session_state.df.empty:
    ...
    with col1:
        # Download as Excel button with improved styling
        excel_buffer = io.BytesIO()
        with pd.ExcelWriter(excel_buffer, engine="xlsxwriter") as writer:
            download_df.to_excel(writer, sheet_name="News Data", index=False)
    ...
    with col2:
        # Load next page button with improved styling
        if st.session_state.has_searched:
            def load_more_callback():
                st.session_state.load_more_clicked = True
            
            button_key = f"load_more_button_{st.session_state.button_key}"
            st.button("➕ Load More", key=button_key, on_click=load_more_callback, use_container_width=True)
    ...
    # ===================== Charts Section =====================
    st.markdown('<div class="section-header"><h3>Overall Tone Summary</h3></div>', unsafe_allow_html=True)
    ...
    # 2. Pie Chart (Interactive) with improved styling
    pie_fig = px.pie(
        names=counts.index,
        values=counts.values,
        title="Overall Sentiment Distribution",
        color=counts.index,
        color_discrete_map={'Positive': '#4CAF50', 'Neutral': '#78909C', 'Negative': '#F44336'},
        hole=0.4
    )
    ...
    # 3. Portal Statistics (interactive stacked bar)
    st.markdown('<div class="section-header"><h3>Portal Statistics</h3></div>', unsafe_allow_html=True)
    ...
    for sentiment in ['Negative', 'Neutral', 'Positive']:
        stacked_fig.add_trace(go.Bar(
            ...
            x=source_stats.index,
            y=source_stats[sentiment],
            marker_color={'Positive': '#4CAF50', 'Neutral': '#78909C', 'Negative': '#F44336'}[sentiment],
            hoverinfo='x+y+name'
        ))
    ...
    # 4. Tone Distribution (%) - interactive horizontal bar
    st.markdown('<div class="section-header"><h3>Tone Distribution by Source (%)</h3></div>', unsafe_allow_html=True)
